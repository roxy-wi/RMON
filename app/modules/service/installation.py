...
def run_ansible(inv: dict, server_ips: str, ansible_role: str) -> object:
	...
	try:
		...
		server_mod.stop_ssh_agent(agent_pid)
		roxywi_common.handle_exceptions(e, 'RMON server', 'Cannot save inventory file', roxywi=1)

	try:
		result = ansible_runner.run(**kwargs)
	except Exception as e:
		server_mod.stop_ssh_agent(agent_pid)
		roxywi_common.handle_exceptions(e, 'RMON server', 'Cannot run Ansible', roxywi=1)

	try:
		server_mod.stop_ssh_agent(agent_pid)
	except Exception as e:
		roxywi_common.logging('RMON server', f'error: Cannot stop SSH agent {e}', roxywi=1)
