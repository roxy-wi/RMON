			...
			if (data.status === 'failed') {
				...
				for (let k in data) {
                    ...
                    if (agent.enabled === 1 && !agent.region_id) {
                        removeAgentFromStatus(agent.id, agent.name);
                    }
                    if (agent.enabled === 1 && Number(agent.region_id) === Number(region_id)) {
                        addAgentToStatus(agent.id, agent.name);
                    }
                }
			}
		}
...
function addAgentToStatus(server_id, hostname, div_id='#checked-agents') {
	...
	let html_tag = '<div id="remove_check-' + server_id + '" data-name="' + hostname + '">' +
		...
		'<div class="add_user_group check-button" onclick="removeAgentFromStatus(' + server_id + ', \'' + hostname + '\')" title="' + delete_word + ' ' + service_word + '">-</div>' +
		'</div>';
	$('#add_check-' + server_id).remove();
	$(div_id).append(html_tag);
}
function removeAgentFromStatus(server_id, hostname, div_id='#all-agents') {
	let add_word = translate_div.attr('data-add');
	let service_word = translate_div.attr('data-service');

	let html_tag = '<div class="all-checks" id="add_check-' + server_id + '" data-name="' + hostname + '">' +
		'<div class="check-name">' + hostname + '</div>' +
		'<div class="add_user_group check-button" onclick="addAgentToStatus(' + server_id + ',  \'' + hostname + '\')" title="' + add_word + ' ' + service_word + '">+</div></div>';
    $(div_id).append(html_tag);
...
function getRegionSettings(agent_id) {
	$.ajax({
		...
		success: function (data) {
			...
			$('#new-region-desc').val(data['description'].replaceAll("'", ""));
			$('#new-regin-enabled').checkboxradio("refresh");
			if (data['enabled'] == '1') {
				$('#new-region-enabled').prop('checked', true)
			} else {
