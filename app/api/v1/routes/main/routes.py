...
def register_api_with_group(view, endpoint, url_beg, pk='receiver', pk_type='int', pk_end='channel_id', pk_type_end='int'):
    ...
    bp.add_url_rule(f'/{url_beg}/<any(telegram, slack, pd, mm, email, incidentrelay):{pk}>', view_func=view_func, methods=['POST'])
    bp.add_url_rule(f'/{url_beg}/<any(telegram, slack, pd, mm, email, incidentrelay):{pk}>/<{pk_type_end}:{pk_end}>', view_func=view_func, methods=['PUT', 'DELETE', 'GET'])


register_api_with_group(ChannelView, 'channel', '/channel')
bp.add_url_rule('/channels/<any(telegram, slack, pd, mm, email, incidentrelay):receiver>', view_func=ChannelsView.as_view('channels'), methods=['GET'])
bp.add_url_rule('/channel/<any(telegram, slack, pd, mm, email, incidentrelay):receiver>/<int:channel_id>/check', view_func=ChannelCheckView.as_view('channel_check'), methods=['POST'])
register_api_with_group(ChannelView, 'rmon_channel', '/rmon/channel')
bp.add_url_rule('/rmon/channels/<any(telegram, slack, pd, mm, email, incidentrelay):receiver>', view_func=ChannelsView.as_view('rmon_channels'), methods=['GET'])
bp.add_url_rule('/rmon/channel/<any(telegram, slack, pd, mm, email, incidentrelay):receiver>/<int:channel_id>/check', view_func=ChannelCheckView.as_view('rmon_channel_check'), methods=['POST'])
...
bp.add_url_rule(
    '/settings/<any(rmon, main, rabbitmq, ldap, monitoring, mail, logs, agent):section>',
